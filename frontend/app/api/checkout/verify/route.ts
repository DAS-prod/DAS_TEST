  ...
  try {
    ...
    const validated = lines.map(
      (line) => {
        ...
        const price = resolveProductPrice(product, line.size);

        return {
          product,
          price,
          size: String(line.size || product.size || ""),
          quantity,
        };
      }
    );
    ...
    if (couponCode && !coupon?.valid) throw new Error(coupon?.message || "Coupon is no longer valid.");
    ...
    if (requestedReferralCode) {
      const { data: influencer, error: influencerError } = await supabase
        .from("influencers")
        .select("referral_code,commission_percent,active")
        .eq("referral_code", requestedReferralCode)
        .eq("active", true)
        .maybeSingle();

      if (!influencerError && influencer?.referral_code) {
        referralCode = requestedReferralCode;
        referralCommissionRate = Number(influencer.commission_percent || 5);
      } else if (influencerError) {
        console.warn("INFLUENCER LOOKUP SKIPPED:", influencerError.message);
      }
    }
    ...
    const featureFields = {
      ...
      referral_commission_rate: referralCode ? referralCommissionRate : 0,
      referral_commission_amount:
        referralCode && referralCommissionRate > 0
          ? Number((((subtotal - discount) * referralCommissionRate) / 100).toFixed(2))
          : 0,
    };

    let orderResult = await supabase
      .from("orders")
      .insert({ ...baseOrder, ...featureFields })
      .select("id,order_number,total_amount,currency")
      .single();

    // Until the V2 migration is applied, local/payment testing can still use the legacy order schema.
    if (orderResult.error && /discount_amount|coupon_code|referral_code|referral_commission|column/i.test(String((orderResult.error as { message?: string }).message || ""))) {
    ...
    const items =
      validated.map(
        ({
          ...
          product_name:
            size ? `${product.name || "Product"} · ${size}` : product.name,
