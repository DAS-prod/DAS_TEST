...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (currentUser) {
        if (requestedNext !== "/" && requestedNext !== "/account") {
    ...
    const {
      ...
      setUser(currentUser);
      if (currentUser) void loadOrders(currentUser.id);
      else setOrders([]);
    });
  ...
  async function loadOrders(userId: string) {
    ...
    try {
      const { data, error } = await supabase
        .from("orders")
        .select(`
          id,order_number,status,payment_status,subtotal,shipping_amount,total_amount,currency,created_at,
          delivery_name,delivery_mobile,delivery_email,
  ...
  async function loginSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!value) throw new Error("Enter your email address or mobile number.");
  ...
  async function signupSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (error) throw error;
      if (!data.user) throw new Error("Account created, but automatic sign-in failed. Please sign in.");
      window.location.href = next;
    } catch (error) {
  ...
  if (!user) {
    return (
      <main className="auth-premium-shell">
        {message && (
          <div className="gb-toast-wrap" role="alert" aria-live="assertive">
          ...
          <section className="auth-premium-card">
            <div className="auth-brand-panel">
              <div className="auth-logo">
                <img src="/logo.png" alt="Godavari Basket" />
              </div>
              <p className="eyebrow light">One account. Every Godavari experience.</p>
              <h1 className="serif">
                Welcome to
                <br /> Godavari Basket
              </h1>
              <p>Use your email or mobile number with the same password, or continue quickly with Google.</p>
            ...
            <div className="auth-form-panel">
              <div className="auth-form-heading">
                ...
                <h2>{mode === "login" ? "Sign in" : "Join Godavari Basket"}</h2>
              </div>

              <button type="button" disabled={busy} onClick={googleLogin} className="auth-google">
              ...
              {mode === "login" ? (
                <form onSubmit={loginSubmit} className="auth-fields">
                  <label>
                ...
                <form onSubmit={signupSubmit} className="auth-fields">
                  <label>
                    <span>Full name</span>
                    <div className="auth-input">
                      <UserRound size={17} />
              ...
              <a className="auth-whatsapp" href={whatsappUrl} target="_blank" rel="noreferrer">
                <MessageCircle size={19} />
                <span>
                  <strong>Having trouble logging in?</strong>
                  <small>Chat with Godavari Basket on WhatsApp</small>
                </span>
              </a>
            </div>
          </section>
  ...
  const displayName = user.user_metadata?.full_name || user.email || user.phone || "Godavari Basket Customer";
    ...
    <main className="min-h-screen bg-cream">
      ...
      <div className="border-b bg-white">
        <div className="container-wide flex items-center justify-between py-5">
          <Link href="/" className="inline-flex items-center gap-2 text-sm font-medium text-forest">
            <ArrowLeft size={17} /> Back to Store
          </Link>
          <button disabled={busy} onClick={signOut} className="inline-flex items-center gap-2 rounded-xl border px-4 py-2 text-sm font-semibold">
      ...
      <div className="container-wide py-8 md:py-12">
        <section className="overflow-hidden rounded-[32px] bg-forest text-white shadow-sm">
          <div className="p-7 md:p-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[.22em] text-white/60">My Account</p>
              <h1 className="serif mt-2 text-3xl md:text-4xl">Hello, {displayName}</h1>
              <p className="mt-2 text-sm text-white/70">Your orders and Godavari Basket account in one place.</p>
            </div>
            <Link href="/" className="inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-semibold text-forest">
        ...
        <section className="mt-8">
          <div className="flex items-end justify-between">
            <div>
              ...
              <h2 className="serif mt-2 text-3xl text-forest">MY ORDERS</h2>
            </div>
            <span className="text-sm text-gray-500">{orders.length} orders</span>
          </div>

          {ordersLoading ? (
            <div className="mt-6 rounded-3xl border bg-white p-10 text-center text-sm text-gray-500">Loading orders...</div>
          ) : orders.length === 0 ? (
            <div className="mt-6 rounded-3xl border bg-white p-10 text-center">
              <Package className="mx-auto text-forest" />
              <h3 className="serif mt-4 text-2xl text-forest">No orders yet</h3>
              <p className="mt-2 text-sm text-gray-500">Your Godavari Basket orders will appear here.</p>
            </div>
          ) : (
            <div className="mt-6 space-y-4">
              {orders.map((order) => (
                <article key={order.id} className="rounded-3xl border bg-white p-5 md:p-6">
                  <button
                    className="flex w-full items-center justify-between gap-4 text-left"
                    onClick={() => setExpanded(expanded === order.id ? null : order.id)}
                  >
                    <div>
                      <p className="text-xs uppercase tracking-[.14em] text-gray-400">{date(order.created_at)}</p>
                      <h3 className="mt-1 font-bold text-forest">{order.order_number}</h3>
                      <p className="mt-1 text-sm text-gray-500">
                        {title(order.status)} · {order.order_items?.reduce((n, i) => n + i.quantity, 0) || 0} items
                      </p>
                    </div>
                      ...
                      {order.order_items?.map((item) => (
                        <div key={item.id} className="flex items-center justify-between gap-3 text-sm">
                          <div className="flex items-center gap-3">
                            {item.product_image ? (
                              <img className="h-12 w-12 rounded-xl object-cover" src={item.product_image} alt="" />
                            ) : (
